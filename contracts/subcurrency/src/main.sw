...
abi Token {
    ...
    #[storage(read)]
    fn balance(address: Address) -> u64;
}
...
configurable {
    MINTER: Address = Address::from(0x9299da6c73e6dc03eeabcce242bb347de3f5f56cd1c70926d76526d7ed199b8b),
}
...
impl Token for Contract {
    #[storage(read, write)]
    fn mint(receiver: Address, amount: u64) {
        if !(msg_sender().unwrap() == Identity::Address(MINTER)) {
            revert(0);
        }
    ...
    #[storage(read)]
    fn balance(address: Address) -> u64 {
        storage.balances.get(address).try_read().unwrap()
    }
}